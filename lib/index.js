	...
	return class NotebooksService extends _classSuper {
		...
		*/
		...
		list(request) {
			...
			return { entries: [...this.requireTable().entries()].map(([, entry]) => snapshot(entry)).filter((entry) => query === void 0 || query === "" || `${entry.title}\n${entry.content}\n${entry.tags.join("\n")}`.toLocaleLowerCase().includes(query)).filter((entry) => tag === void 0 || tag === "" || entry.tags.some((candidate) => candidate.toLocaleLowerCase() === tag)).sort((left, right) => right.updatedAt - left.updatedAt) };
		}
		...
		put(request) {
			return this.enqueue(async () => {
				...
				if (content.length > this.config.maxContentChars) throw new RangeError(`notebooks: content exceeds ${this.config.maxContentChars} characters`);
				...
				const entry = snapshot({
					...
					updatedAt: current === void 0 ? now : Math.max(now, current.updatedAt)
				});
		...
		registerTools() {
			this.ctx.tools.register(defineTool({
				...
				parameters: {
					query: {
						type: "string",
						description: "Optional text matched against titles, content, and tags."
					},
					tag: {
						type: "string",
				...
				output: {
					...
					render: (_args, value) => [{
						type: "text",
						text: value.entries.length === 0 ? "No notebook entries matched." : value.entries.map((entry) => `- ${entry.title} (${entry.id})\n  ${entry.content}`).join("\n")
					}]
				},
			...
			this.ctx.tools.register(defineTool({
				...
				parameters: {
					id: {
						type: "string",
						description: "Existing notebook id to replace; omit to create."
					},
					title: {
						type: "string",
						required: true,
						description: "Short, specific entry title."
					},
					content: {
						type: "string",
						required: true,
				...
				output: {
					...
					render: (_args, value) => [{
						type: "text",
						text: `Saved notebook entry "${value.title}" (${value.id}).`
					}]
				},
				...
				presentCall: (args) => ({
					card: "generic",
					kind: "edit",
					title: args.id === void 0 ? "Create note" : "Update note",
					rawInput: args.title
				})
			}));
			this.ctx.tools.register(defineTool({
				...
				parameters: { id: {
					type: "string",
					required: true,
					description: "Notebook id returned by notebook_list."
				} },
				output: {
					schema: {
						type: "object",
					...
					render: (args, value) => [{
						type: "text",
						text: value.deleted ? `Deleted notebook entry ${args.id}.` : `Notebook entry ${args.id} was already absent.`
					}]
				},
...
//#endregion
export { NotebookId, NotebooksService, NotebooksService as default, notebookEntrySchema, notebooksDomainSpec };
